...
crack = geom.line(P1=-a*np.exp(1j*alpha)-1j+1, P2=a*np.exp(1j*alpha)-1j+1, bc_type=bc.stress_bc(), bc_value=0., on_boundary=False)
interface1 = geom.line(P1=[-l,(-l-1)*np.tan(alpha)-1],P2=-a*np.exp(1j*alpha)-1j+1, bc_type=bc.interface_bc())
interface2 = geom.line(P1=a*np.exp(1j*alpha)-1j+1,P2=[l,(l-1)*np.tan(alpha)-1], bc_type=bc.interface_bc())

def dd_partition (x,y): # Domain decomposition partition
    return torch.stack([
    (y+1-np.tan(alpha)*(x-1)>=-1e-10),
    (y+1-np.tan(alpha)*(x-1)<=1e-10)
    ])

crack.add_crack_tip(tip_side=0, initial_sif=12.0)
crack.add_crack_tip(tip_side=1, initial_sif=12.0)
boundary = geom.boundary([line1, line2, line3, line4, crack, interface1, interface2], np_train, np_test, dd_partition, enrichment='williams')
